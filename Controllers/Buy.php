...
switch ($_GET["op"]) {
	case 'guardaryeditar':
		if (empty($idingreso)) {
			$rspta = $buy->insertar($idproveedor, $idusuario, $tipo_comprobante, $serie_comprobante, $num_comprobante, $fecha_hora,  $total_compra, $_POST["idarticulo"], $_POST["cantidad"], $_POST["precio_compra"], $_POST["precio_venta"]);
			echo $rspta ? "Datos registrados correctamente" : "No se pudo registrar los datos";
		} else {
			$rspta = $buy->editar($idingreso, $idproveedor, $tipo_comprobante, $serie_comprobante, $num_comprobante, $fecha_hora,  $total_compra, $_POST["idarticulo"], $_POST["nuevostock"], $_POST["cantidad"], $_POST["precio_compra"], $_POST["precio_venta"]);
			echo $rspta ? "Datos registrados correctamente" : "No se pudo registrar los datos";
		}
		break;
	...
	case 'listarDetalle':
		...
		echo '<tfoot>
        <th><span>SubTotal</span><br><br><span>TOTAL</span></th>
         <th></th>
         <th></th>
         <th></th>
         <th><span id="total">' . $smoneda . ' ' . number_format((float)$total, 2, '.', '') . '</span><br>' . $smoneda . ' ' . number_format((float)$most_igv, 2, '.', '') . '</span><br><span id="most_total" maxlength="4">' . $smoneda . ' ' . $t_compra . '</span></th>
       </tfoot>';
		break;
	...
	case 'listar':
		...
		foreach ($rspta as $reg) {
			$data[] = array(
				...
				"6" => $reg['total_compra'],
				"7" => ($reg['estado'] == 'Aceptado') ? '<span class="badge bg-success">Aceptado</span>' : '<span class="badge bg-danger">Anulado</span>'
			);
		}
		$results = array(
			"sEcho" => 1, 
