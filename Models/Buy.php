...
class Buy
{
    ...
    {
        $sql = "INSERT INTO $this->tableName (idproveedor,idusuario,tipo_comprobante,serie_comprobante,num_comprobante,fecha_hora,total_compra,estado) VALUES (?,?,?,?,?,?,?,?)";
        $arrData = array($idproveedor, $idusuario, $tipo_comprobante, $serie_comprobante, $num_comprobante, $fecha_hora,  $total_compra, 'Aceptado');
    ...
    //FUNCION PARA EDITAR
    public function editar($idingreso, $idproveedor, $tipo_comprobante, $serie_comprobante, $num_comprobante, $fecha_hora,  $total_compra, $idarticulo, $nuevostock, $cantidad, $precio_compra, $precio_venta)
    {
        $sw = true;
        $sql = "UPDATE $this->tableName SET idproveedor=?, tipo_comprobante=?, serie_comprobante=?, num_comprobante=?, fecha_hora=?,  total_compra=? WHERE idingreso=?";

        $arrData = array($idproveedor, $tipo_comprobante, $serie_comprobante, $num_comprobante, $fecha_hora,  $total_compra, $idingreso);
    ...
    {
        $sql = "SELECT i.idingreso,DATE(i.fecha_hora) as fecha,i.idproveedor,p.nombre as proveedor,u.idusuario,u.nombre as usuario, i.tipo_comprobante,i.serie_comprobante,i.num_comprobante,i.total_compra,i.estado FROM $this->tableName i INNER JOIN persona p ON i.idproveedor=p.idpersona INNER JOIN usuario u ON i.idusuario=u.idusuario WHERE idingreso=?";
        $arrData = array($idingreso);
    ...
    {
        $sql = "SELECT di.idingreso,di.idarticulo,a.stock,a.nombre,di.cantidad,di.precio_compra,di.precio_venta,i.total_compra FROM $this->tableNameDetalle di INNER JOIN articulo a ON di.idarticulo=a.idarticulo INNER JOIN $this->tableName i ON i.idingreso=di.idingreso WHERE di.idingreso='$idingreso'";
