...
class User{
	...
	public function editar($idusuario, $nombre, $tipo_documento, $num_documento, $direccion, $telefono, $email, $cargo, $login, $imagen, $permisos) {
		$sw = true;
		$actualizacionultima = date("Y-m-d H:i:s"); // Fecha y hora actual
		$sql = "UPDATE $this->tableName SET nombre=?, tipo_documento=?, num_documento=?, direccion=?, telefono=?, email=?, cargo=?, login=?, imagen=?, actualizacionultima=? WHERE idusuario=?";
		$arrDataUpdate = array($nombre, $tipo_documento, $num_documento, $direccion, $telefono, $email, $cargo, $login, $imagen, $actualizacionultima, $idusuario);
	...
	public function editarPerfil($idusuario,$nombre,$tipo_documento,$num_documento,$direccion,$telefono,$email,$login,$clave,$imagen,$descripcion,$biografia){
		$sw=true;
		$actualizacionultima=date("Y-m-d H:i:s");
		...
		WHERE idusuario=?";
		$arrDataUpdate = array($nombre,$tipo_documento,$num_documento,$direccion,$telefono,$email,$login,$imagen,$descripcion,$biografia,$idusuario,$actualizacionultima);
		$this->conexion->setData($sql,$arrDataUpdate) or $sw=false;
		if(!empty($clave)){
